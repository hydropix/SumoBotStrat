...
const S = {
    kp:                 ARGS.kp             ?? 0.8,
    searchPwm:          ARGS.searchPwm      ?? 0.75,  // fraction of maxPwm
    trackPwm:           ARGS.trackPwm       ?? 0.8,
    chargePwm:          ARGS.chargePwm      ?? 1.0,
    chargeThreshold:    ARGS.chargeThreshold ?? 200,   // mm
    ...
    evadePwm:           ARGS.evadePwm        ?? 0.7,
    edgeSteer:          ARGS.edgeSteer       ?? 0.5,   // steering correction when single front sensor on line during charge
...
const P = {
    ...
    laserRange: 0.80, laserOffY: 0.025, laserAngle: ARGS.laserAngle ?? 0,  // radians, outward splay per sensor
...
function motorForce(pwm, wheelLinSpeed) {
    const targetSpeed = (pwm / 255) * P.noLoadSpeed;
...
function physicsStep(r, dt) {
    ...
    if (abs(fL) > P.tractionMax) fL = P.tractionMax * sign(fL);
    if (abs(fR) > P.tractionMax) fR = P.tractionMax * sign(fR);

    const fFwd = fL + fR - P.rollDamping * vFwd * P.mass;
    const torque = (fL - fR) * P.wheelBase / 2 - P.angDamping * r.omega;
    const latMax = P.mu * P.mass * P.g * 1.5;
    const fLat = clamp(-P.latDamping * vLat * P.mass, -latMax, latMax);
...
function resolveCollision(a, b) {
    ...
    if (dvn <= 0) return;
    const j = (1 + P.restitution) * dvn / (2 / P.mass);
    a.vx -= (j / P.mass) * nx; a.vy -= (j / P.mass) * ny;
    b.vx += (j / P.mass) * nx; b.vy += (j / P.mass) * ny;
}

function isOut(r) { return hypot(r.x, r.y) > P.arenaR + P.colRadius * 0.5; }
...
function getLaser(r, tgt) {
    ...
    return {
        d1: rayCast(lp.x, lp.y, r.a - P.laserAngle, tgt.x, tgt.y, P.colRadius, P.laserRange),
        d2: rayCast(rp.x, rp.y, r.a + P.laserAngle, tgt.x, tgt.y, P.colRadius, P.laserRange),
    };
}
...
function makeRobot(x, y, a) {
    return { x, y, a, vx: 0, vy: 0, omega: 0, cmdL: 0, cmdR: 0 };
}
...
function runRound() {
    ...
    const bot = makeRobot(cos(bAng) * bR, sin(bAng) * bR, bFace);
    const ene = makeRobot(cos(eAng) * eR, sin(eAng) * eR, eFace);
